...
public class Dronning extends Brikke {
	...
	public Dronning(Rute rute, Farge farge, Parti parti) {
		super(rute, farge, parti, 72);
	}
	
	@Override
	public void nyInstanse(Rute rute, Farge farge, Parti parti) {
		new Dronning(rute, farge, parti).setUroert(uroert);
	}
	...
	public boolean angriperRute(Rute rute) {
		...
		int yDifferanse = Math.abs(y1-y2);
		
		if (x1 != x2 && y1 != y2 && xDifferanse != yDifferanse) return false;
